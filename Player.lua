    ...
    self.behaviors = {
        ...
        ['walking'] = function(dt)
            ...
            elseif love.keyboard.isDown('a') and self.x>0 then
                self.direction = 'left'
                self.dx = -WALKING_SPEED
            elseif love.keyboard.isDown('d') and self.x+self.width<map.mapWidthPixels then
                self.direction = 'right'
                self.dx = WALKING_SPEED
            else
                self.dx = 0
...
function Player:update(dt)
    ...
    --generates new level when mario passes the flag
    if  self.x+self.height>VIRTUAL_WIDTH+30   then
        map.music:stop()
        map:init()
    end
end
